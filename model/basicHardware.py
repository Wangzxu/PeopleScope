...
class BasicHardware(Base):
    ...
    # 社会经济指标
    education = Column(Integer, comment="最高学历，用数字存储，比如3代表本科，4代表硕士，5代表博士")
...
class FriendHardware(Base):
    ...
    user = Column(String(64), nullable=False, comment="用户名")
    # 基础数值指标
    birth_year_min = Column(Integer, comment="期望最小出生年份，示例: 1995")
    birth_year_max = Column(Integer, comment="期望最大出生年份，示例: 2000")
    height_min = Column(Integer, comment="期望最小身高(cm)，示例: 160")
    height_max = Column(Integer, comment="期望最大身高(cm)，示例: 180")
